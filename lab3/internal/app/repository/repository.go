...
func (r *Repository) migrate() error {
	...
	stmts := []string{
		...
		"ALTER TABLE reference_alloy_services ADD COLUMN IF NOT EXISTS clip_description_en VARCHAR(255) NOT NULL DEFAULT ''",
		"UPDATE reference_alloy_services SET clip_description_en = '' WHERE clip_description_en IS NULL",
		"UPDATE reference_alloy_services SET clip_description_en = 'Dark brown bronze ingot with oxidized rough texture and warm copper highlights under light.' WHERE clip_description_en = '' AND LOWER(slug) LIKE '%bronze%'",
		"UPDATE reference_alloy_services SET clip_description_en = 'Warm yellow brass ingot with bright golden edges and medium metallic reflectance on surface.' WHERE clip_description_en = '' AND LOWER(slug) LIKE '%brass%'",
		"UPDATE reference_alloy_services SET clip_description_en = 'Dark gray iron ingot with matte grainy texture, cold tone, and weak silver highlights.' WHERE clip_description_en = '' AND LOWER(slug) LIKE '%iron%'",
		"UPDATE reference_alloy_services SET clip_description_en = 'Pale silver ingot with smooth texture, strong reflectance, and bright white specular highlights.' WHERE clip_description_en = '' AND LOWER(slug) LIKE '%silver%'",
		"UPDATE reference_alloy_services SET clip_description_en = 'Archaeological metal reference alloy sample for XRF spectral comparison and CLIP similarity search.' WHERE clip_description_en = ''",
...
func upsertService(db *gorm.DB, service model.ReferenceAlloyService) error {
	...
	if updateErr := db.Model(&existing).Updates(map[string]any{
		...
		"clip_description_en": service.ClipDescriptionEN,
